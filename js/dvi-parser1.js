...
function dvipass1(arr) {
  ...
  for (var ptr = 0; ptr < len; ) {
    ...
    if (op < 128) {         //   0..127
      ...
      case 132: // setrule
        name = "setrule";
        var a = readu(arr, ptr, 4); ptr += 4;
        var b = readu(arr, ptr, 4); ptr += 4;
        args = "a="+ a + " b="+ b;
      ...
      case 139: // bop
        name = "bop";
        for (var i = 0; i < 10; i++) {
          c[i] = readu(arr, ptr, 4); ptr += 4;
        }
        var p = readu(arr, ptr, 4); ptr += 4;
        h = v = w = x = y = 0;
        st = [];
        args = "...";
      ...
      case 243: case 244: case 245: case 246: // fntdef
        ...
        name = "fntdef"+ j;
        var k = readu(arr, ptr, j); ptr += j;
        var c = readu(arr, ptr, 4); ptr += 4;
        var s = readu(arr, ptr, 4); ptr += 4;
        var d = readu(arr, ptr, 4); ptr += 4;
        var a_ = arr[ptr++];
        var l_ = arr[ptr++];
        var n = reads(arr, ptr, a_+l_); ptr += a_+l_;
      ...
      case 247: // pre
        ...
        var num = readu(arr, ptr, 4); ptr += 4;
        var den = readu(arr, ptr, 4); ptr += 4;
        var unit = num/den*1e-7;
        var mag = readu(arr, ptr, 4)/1000; ptr += 4;
        var k = arr[ptr++];
        var comment = reads(arr, ptr, k); ptr += k;
      ...
      case 248: // post
        ...
        var num = readu(arr, ptr, 4); ptr += 4;
        var den = readu(arr, ptr, 4); ptr += 4;
        var mag = readu(arr, ptr, 4); ptr += 4;
        var l_ = readu(arr, ptr, 4); ptr += 4;
        var u_ = readu(arr, ptr, 4); ptr += 4;
        var s_ = readu(arr, ptr, 2); ptr += 2;
        var t_ = readu(arr, ptr, 2); ptr += 2;
        args = "p="+ p +" num="+ num +" den="+ den +" unit="+ unit +" mag="+ mag +" l="+ l_ +" u="+ u_ +" s="+ s_ +" t="+ t_;
        rem = "ポストアンブル始まり";
        break;
      case 249: // post_post
        name = "post_post";
        var q = readu(arr, ptr, 4); ptr += 4;
        var i = arr[ptr++]; 
        rem = "ポストアンブル終わり";
        break;
